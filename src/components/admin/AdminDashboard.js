...
export function AdminDashboard({ submissions: initialSubmissions, projects: initialProjects, attendees: initialAttendees = [] }) {
  const [activeTab, setActiveTab] = useState('hackathon'); // 'hackathon' | 'projects' | 'attendees'
  ...
  const handleDelete = async () => {
    ...
    const isAttendee = deleteModal.type === 'attendee';
    const endpoint = isProject ? '/api/admin/projects' : isAttendee ? '/api/admin/attendees' : '/api/admin/hackathon';
    ...
    try {
      ...
      if (res.ok) {
        if (isProject) {
          ...
          setAttendees(prev => prev.filter(s => s.id !== deleteModal.item.id));
        } else {
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 font-display text-4xl font-bold text-white">
            {activeTab === 'hackathon' ? 'Hackathon Başvuruları' : activeTab === 'projects' ? 'Demo Day Projeleri' : 'Ziyaretçi Kayıtları'}
          </h1>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="flex bg-white/5 rounded-full p-1 border border-white/10 sm:mr-4">
            ...
            <button 
              ...
              className={`px-6 py-2 rounded-full text-sm font-semibold transition ${activeTab === 'attendees' ? 'bg-white text-black' : 'text-white hover:bg-white/5'}`}
            >
              Ziyaretçiler
            </button>
          </div>
              ...
              <table className="min-w-full divide-y divide-white/10 text-left text-sm">
                ...
                <tbody className="divide-y divide-white/8">
                  ...
                  {attendees.map((attendee) => (
                    <tr key={attendee.id} className="align-top hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-4">
                        <div className="font-semibold text-white">{attendee.fullName}</div>
                        <div className="mt-1 text-ink-dim text-xs">{attendee.university || '-'}</div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="text-white">{attendee.email}</div>
                      ...
                      <td className="px-4 py-4">
                        <button
                          onClick={() => openDeleteModal(attendee, 'attendee')}
                          className="p-2 text-ink-dim hover:text-red-400 transition-colors rounded-lg hover:bg-red-400/10"
                        >
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M10 11v6M14 11v6" /></svg>
                        </button>
