  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-12">
      <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 items-start">
        ...
        {/* SOL PANEL: Bilgi ve Duyuru (Daha Açık Renk Teması) */}
        <aside className="w-full lg:w-[420px] shrink-0 lg:sticky lg:top-24">
          <div className="relative overflow-hidden rounded-[3rem] border border-white/20 bg-white/[0.07] p-10 lg:p-12 shadow-2xl backdrop-blur-xl">
            {/* Dekoratif Parlama */}
            <div className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-primary/20 blur-[100px]"></div>
            
            <div className="relative z-10 space-y-12">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full bg-primary/20 px-4 py-2 text-xs font-black uppercase tracking-[0.25em] text-primary mb-6">
                  <span className="h-2 w-2 animate-pulse rounded-full bg-primary"></span>
                  Önemli Bilgi
                </div>
                <h2 className="font-display text-4xl lg:text-5xl font-black leading-[1.1] text-white">
                  HackFest&apos;26 <br/>
                  <span className="text-primary/90">AI Edition</span>
                </h2>
              </div>

              <div className="space-y-10">
                <InfoItem icon="📍" title="Fiziksel Katılım">
                  <p className="text-red-400 font-black text-base leading-relaxed">
                ...
                <InfoItem icon="💡" title="Proje Teması">
                  <p className="text-base leading-relaxed text-ink-dim">Eğitim, sağlık ve çevre gibi alanlarda toplum yararına yapay zeka çözümleri geliştirin.</p>
                </InfoItem>

                <InfoItem icon="🔗" title="Detaylar">
                  <p className="text-base leading-relaxed text-ink-dim">
                    Ödüller, mentorlar ve kurallar hakkında tüm detaylara üst menüden ulaşabilirsiniz.
                  </p>
                </InfoItem>
              </div>

              <div className="pt-10 border-t border-white/10">
                <div className="text-xs font-black text-ink-mute uppercase tracking-[0.2em] mb-6">Resmi WhatsApp Kanalı</div>
                <a
                  href={SITE.social.whatsapp}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group flex items-center justify-between gap-4 rounded-[2rem] bg-[#25D366] px-10 py-6 text-lg font-black text-white transition-all hover:scale-[1.03] active:scale-95 shadow-[0_20px_50px_-15px_rgba(37,211,102,0.5)]"
                >
          ...
          <form onSubmit={submit} noValidate className="space-y-20">
            ...
            <FormSection title="Proje Fikri ve İlgi Alanı">
              <div className="space-y-6">
                ...
                />
                <p className="text-sm text-ink-mute italic leading-relaxed px-2 border-l-2 border-primary/30">
            ...
            <div className="pt-12">
              ...
              <p className="mt-10 text-center text-sm text-ink-mute leading-relaxed max-w-lg mx-auto">
...
function InfoItem({ icon, title, children }) {
  return (
    <div className="flex gap-6 group">
      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-3xl shadow-xl border border-white/20 transition-all group-hover:bg-white/15">
        {icon}
      </div>
      <div className="space-y-2">
        <div className="text-sm font-black text-white tracking-[0.15em] uppercase">{title}</div>
        <div className="text-base leading-relaxed text-ink-dim/90">{children}</div>
      </div>
    </div>
  );
}
